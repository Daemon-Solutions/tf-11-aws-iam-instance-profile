resource "aws_iam_role_policy" "s3_readonly" {
  name  = "s3_read-${element(split(",", var.s3_read_buckets), count.index)}"
  count = "${var.s3_read_buckets != "" ? length(split(",", var.s3_read_buckets)) : 0}"
  ...
  "Statement": [
    {
      ...
      "Effect": "Allow",
      "Resource": [
        "arn:aws:s3:::${element(split(",", var.s3_read_buckets), count.index)}",
        "arn:aws:s3:::${element(split(",", var.s3_read_buckets), count.index)}/*"
      ]
    }
  ]
